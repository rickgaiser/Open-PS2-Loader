...
EE_LIBS = -L$(PS2SDK)/ports/lib -L$(GSKIT)/lib -L./lib -lgskit -ldmakit -lgskit_toolkit -lpoweroff -lfileXio -lpatches -ljpeg -lpng -lzlib -ldebug -lm -lmc -lfreetype -lvux -lcdvd -lnetman -lps2ips
...
release:
	echo "Building Open PS2 Loader $(OPL_VERSION)..."
	echo "-Interface"
	$(MAKE) VMC=1 GSM=1 IGS=1 PADEMU=0 CHEAT=1 HIRES=1 $(EE_VPKD).zip
